...
def crearDB (registro):
    try:
        archivo=open("registros_empleados.txt","w")#primero ponemos el nombre del archivo y extension y se le da el prermiso w para escribir
...
def registrar_cliente():#
    ...
    nuevo_empleado = pers.to_dict()#esto hace que se guarde en un dicionario
    for registro in datos:#aca hacemos la comprabacion de datos para evitar que se repita una cedula
        if registro["Cedula"]== nuevo_empleado["Cedula"]:#en esta linea se comprueba que no  se encuentren la cedula ingresada con una anterior que siga presente
            print("la cedula ya se encuentra en el sistema ")
            return
    datos.append(nuevo_empleado)#si no se encuentra la cedula se logra registrar en el sistema
...
def ver_registro ():#esta funcion lo que hace es leer los datos de la listra mostrandolo al empleado para que este vea la lista con los datos
     if os.path.exists ("registros_empleados.txt"):#esto lo que hace es cuando la lista esta creada muestre los datos
         registro = open("registros_empleados.txt","r")#con esa r se le da el permiso para leer el txt y mostrarlo
...
def backup():#hace un respaldo de los archivos cada vez que se sale del sistema
    ...
    if not os.path.exists("registros_empleados.txt"):#si esa carpeta existe devuelve lo siguiente
        return
    fecha = datetime.now().strftime("%Y-%m-%d_%H-%M")#fecha y hora en el txt
    nombredelrespaldo = f"{carp}/backup_{fecha}.txt"
    shutil.copy("registros_empleados.txt", nombredelrespaldo)#hace una copia dentro de la carpeta con nombre de backup
